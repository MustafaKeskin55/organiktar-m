-- ============================================
-- ORGANIK TARIM - FINAL DATABASE SCHEMA
-- Tarih: 2026-04-02
-- Aciklama: Tüm tablolar - Trigger çakışması düzeltildi
-- ============================================

-- ============================================
-- 0. ONCELIKLE TUM TABLOLARI SIL
-- ============================================
...
INSERT INTO site_settings (site_name, site_description, contact_email, contact_phone, commission_rate, shipping_fee) VALUES
('Organik Tarım', 'Doğal ve organik ürünlerin buluşma noktası', 'info@organiktarim.com', '0850 123 45 67', 5.00, 29.99);

-- ============================================
-- 3. ODEME AYARLARI TABLOSU
-- ============================================
...
-- ============================================
-- 8. SIPARISLER TABLOSU - Trigger olmadan
-- ============================================
CREATE TABLE orders (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
...
-- Demo siparişler - elle order_number vererek
INSERT INTO orders (order_number, user_id, address_id, total_amount, delivery_fee, status, payment_method, payment_status, estimated_delivery) VALUES
('ORD-001-2026-04', 4, 1, 150.00, 29.99, 'TESLIM_EDILDI', 'CARD', 'COMPLETED', DATE_ADD(CURDATE(), INTERVAL -2 DAY)),
('ORD-002-2026-04', 4, 1, 250.00, 0.00, 'TESLIM_EDILDI', 'TRANSFER', 'COMPLETED', DATE_ADD(CURDATE(), INTERVAL -5 DAY)),
('ORD-003-2026-04', 5, 3, 75.00, 29.99, 'YOLDA', 'COD', 'PENDING', DATE_ADD(CURDATE(), INTERVAL 1 DAY)),
('ORD-004-2026-04', 4, 2, 500.00, 0.00, 'HAZIRLANIYOR', 'CARD', 'COMPLETED', DATE_ADD(CURDATE(), INTERVAL 2 DAY)),
('ORD-005-2026-04', 5, 3, 45.00, 29.99, 'BEKLEMEDE', 'TRANSFER', 'PENDING', DATE_ADD(CURDATE(), INTERVAL 3 DAY));
...
CREATE INDEX INDEX idx_users_type ON users(type);
...
SELECT CONCAT('📊 Total Tables: ', COUNT(*)) as info FROM information_schema.tables WHERE table_schema = DATABASE();
